...
local client_menu                       = require("widgets.radical.impl.tasklist.client_menu")
local theme                             = require("widgets.radical.theme")
local rad_client                        = require("widgets.radical.impl.common.client")
local shape                             = require("gears.shape")

local sticky, urgent, instances, module = { extensions = require( "widgets.radical.impl.tasklist.extensions") }, {}, {}, {}
...
local function load_clients(t)
    ...
    if not t or not screen or not instances[capi.screen[screen]] then
        return
    end
    local menu     = instances[capi.screen[screen]].menu
    local clients  = {}
...
local function untagged(c, t)
    ...
    if not item or not instances[capi.screen[screen]] then
        return
    end
    local menu = instances[capi.screen[screen]].menu
    if t.selected then
