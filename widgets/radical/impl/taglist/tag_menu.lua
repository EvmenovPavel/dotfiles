...
local io           = io
local menu         = require("widgets.radical.context")
local beautiful    = require("beautiful")
local com_tag      = require("widgets.radical.impl.common.tag")
local radical      = require( "widgets.radical")
local extensions   = require( "widgets.radical.impl.taglist.extensions")
...
local function color_menu(t)
    if not tag_list then
        tag_list = require( "widgets.radical.impl.taglist")
    end
    local m = radical.context { layout = radical.layout.grid, column = 6 }
    m:add_item { icon = gen_icon("#ff0000", m.item_height - 4), item_layout = radical.item.layout.icon, button1 = function()
        set_color(t, "#ff0000")
    end }
    m:add_item { icon = gen_icon("#00ff00", m.item_height - 4), item_layout = radical.item.layout.icon, button1 = function()
        set_color(t, "#00ff00")
    end }
...
local function new(t)
    ...
    aTagMenu:add_item({ text = "Visible", checked = true, button1 = function()
        aTag.selected = not aTag.selected
    end })
    aTagMenu:add_item({ text = "Rename", button1 = function()
        --[[shifty.rename(aTag)]]
    end })
    ...
    if capi.screen.count() > 1 then
        ...
        for i = 1, capi.screen.count() do
            screenMenu:add_item({ text = "Screen " .. i, checked = aTag.screen == i, button1 = function()
                --[[tag_to_screen(aTag,i)]]
            end })
        end
    end

    aTagMenu:add_item({ text = "Set color", sub_menu = function()
        return color_menu(aTag)
    end })
    aTagMenu:add_item({ text = "Merge With", sub_menu = function()
        return com_tag.listTags()
    end })
    ...
    while true do
        ...
        mainMenu2:add_item({ button1 = function()
            aTag.icon = file
        end, icon                    = file, item_layout = radical.item.layout.icon })
        counter = counter + 1
    end
    f:close()
