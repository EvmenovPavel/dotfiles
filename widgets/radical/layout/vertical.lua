...
local scroll       = require("widgets.radical.widgets.scroll")
local filter       = require("widgets.radical.widgets.filter")
...
function module:setup_item(data, item, args)
    ...
    if not item._internal.margin_w then
        return
    end
    ...
    if data.auto_resize then
        --FIXME this wont work if the text change
