...
local object        = require("widgets.radical.object")
local theme         = require("widgets.radical.theme")
local item_mod      = require("widgets.radical.item")
...
local function activateKeyboard(data)
    ...
    if (not (data._internal.private_data.enable_keyboard == false)) and data.visible == true then
        capi.keygrabber.run(function(mod, key, event)
            for k, v in pairs(data._internal.filter_hooks or {}) do
                --TODO modkeys
...
local function new(args)
    ...
    if not internal.items then
        internal.items = {}
    end
    if not internal.widgets then
        internal.widgets = {}
    end
    ...
    local data, private_data                                                                    = object({
                                                                                                             private_data    = {
                                                                                                                 ...
                                                                                                                 item_style            = args.item_style or beautiful.menu_default_item_style or require( "widgets.radical.item.style.basic"),
    ...
    -- Getters
    data.get_is_menu               = function(_)
        return true
    end
    data.get_margin                = function(_)
        return { top = 0, bottom = 0, right = 0, left = 0 }
    end
    data.get_items                 = function(_)
        return internal.items
    end
    data.get_rowcount              = function(_)
        return #internal.items
    end
    ...
    -- Setters
    data.set_auto_resize           = function(_, val)
        private_data[""] = val
    end
    ...
    function data:swap(item1, item2)
        --TODO can item.index be used?
        if not item1 or not item2 then
            return
        end
        if not item1 or not item2 and item1 ~= item2 then
            return
        end
        local idx1, idx2
        for k, v in ipairs(internal.items) do
