...
local horizontal_item_layout = require("widgets.radical.item.layout.horizontal")
local margins2               = require("widgets.radical.margins")
...
local function up(data)
    if not data.previous_item then
        return
    end
...
function module.setup_item(data, item, args)
    ...
    if data.max_items ~= nil and data.rowcount > data.max_items then
        -- and (data._start_at or 0)
