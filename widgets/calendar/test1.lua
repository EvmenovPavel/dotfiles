...
local function span_wrap(str, color)
...
function calendar:page(month, year)
    ...
    page = page .. "\n" .. span_wrap(format_date(self.week_col, tA), self.week_color)
    ...
    for day = 1, dB.day do
        if column == 7 then
            column = 0
            nLines = nLines + 1
            page   = page .. "\n" .. span_wrap(format_date(self.week_col, { year = year, month = month, day = day }), self.week_color)
        end
