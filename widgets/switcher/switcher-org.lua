...
function _M.switch(dir, mod_key1, release_key, mod_key2, key_switch)
    -- dir - принимает значение (1 / -1), оно перекидывает на след / пред окно (client)

    -- TODO
    -- сделать так, что, когда вызываешь меню программ, оно работает только когда удерживаешь Alt [Key] клавишу
    -- [Key] - можно назначить
    -- когда отпускаешь, то, меню исчезает
    ...
    keygrabber.run(
            function(mod, keys, event)
                ...
                if gears.table.hasitem(mod, mod_key1) then
                    if keys == "Escape" and event == "release" then
                        _M.close(keys, event)
                    elseif keys == key_switch and event == "press" then
                        _M.next(mod, mod_key2)
                    end
