...
local function persistence_menu(ext, position)
    if not taglist then
        taglist = require( "widgets.radical.impl.taglist")
    end
    if not per_m then
...
local function extension_list_menu(position)
    ...
    if not ext_list_m then
        ...
        for k, v in pairs(extension_list) do
            ext_list_m:add_item { text = k, sub_menu = function()
                return persistence_menu(v, current_pos)
            end }
        end
    end
    return ext_list_m
...
function module.extensions_menu(c)
    ...
    if not ext_m then
        ...
        ext_m:add_item { text = "Overlay widget", sub_menu = function()
            return extension_list_menu("overlay")
        end }
        ext_m:add_item { text = "Prefix widget", sub_menu = function()
            return extension_list_menu("prefix")
        end }
        ext_m:add_item { text = "Suffix widget", sub_menu = function()
            return extension_list_menu("suffix")
        end }
    end
    return ext_m
end
...
function module.add(name, f)
    ...
    if ext_list_m then
        ext_list_m:add_item { text = name, sub_menu = function()
            return persistence_menu(f)
        end }
    end
end
