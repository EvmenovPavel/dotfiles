...
local listTags         = require("widgets.radical.impl.common.tag").listTags
local singalMenu       = require("widgets.radical.impl.common.client").signals
local extensions       = require( "widgets.radical.impl.tasklist.extensions")
...
local function new(screen, args)
    ...
    itemVisible   = mainMenu:add_item {
        ...
        checked = function()
            if module.client ~= nil then
                return not module.client.hidden
            else
                return false
            end
        end,
    ...
    mainMenu:add_item { text = "Move to tag", sub_menu = function()
        return listTags()
    end, }
    mainMenu:add_item { text = "Send Signal", sub_menu = singalMenu(), }
    mainMenu:add_item { text = "Renice", checked = true, button1 = function()
    end, }
    ...
    mainMenu:add_item({ text = "Layer", sub_menu = layerMenu(), button1 = function()
    end })
    mainMenu:add_item { text = "Add widgets", sub_menu = function()
        return extensions.extensions_menu(module.client)
    end }
    mainMenu:add_widget(radical.widgets.separator())

    local ib = wibox.widget.imagebox()
    ib:set_image(beautiful.titlebar_close_button_normal)
    mainMenu:add_item({ text = "Close", suffix_widget = ib, button1 = function()
        if module.client ~= nil then
            module.client:kill();
            mainMenu.visible = false
        end
