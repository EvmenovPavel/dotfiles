...
local pcommon     = require("autofocus._common")
...
function log(permissions, c)
    ...
    if c then
        str = "\n" .. tostring(c.tag) .. "\n" ..
                ...
                tostring(c.class) .. "\n" ..
                tostring(c.screen) .. "\n" ..
                tostring(c.exec_once) .. "\n"

        capi.log:message(permissions, str)
        return
    end

    capi.log:message(permissions)
end
...
local function check_focus_tag(t)
    ...
    if capi.client.focus and capi. screen[capi.client.focus.screen] ~= s then
...
--  with the client.
function permissions.activate(c, context, hints)
    log("permissions.activate")
    ...
    if hints.raise then
        c:raise()
        if not capi.awesome.startup and not c:isvisible() then
            c.urgent = true
        end
    end
    ...
    if hints.switchtotag or hints.switch_to_tag or hints.switch_to_tags then
        atag.viewmore(c:tags(), c.screen, (not hints.switch_to_tags)
                and 0 or nil)
    end
end
...
function permissions.add_activate_filter(f, context)
    if not context then
        ...
        permissions.contextual_activate_filters[context] = permissions.contextual_activate_filters[context] or {}
...
function permissions.tag(c, t, hints)
    log("permissions.tag")
...
function permissions.urgent(c, urgent)
    if c ~= capi.client.focus and not aclient.property.get(c, "ignore_urgent") then
        c.urgent = urgent
    end
end
...
function permissions.autoactivate(c, context, args)
    -- срабатывает ри наведении без клика
    log("permissions.autoactivate")
...
capi.client.connect_signal("request::autoactivate", permissions.autoactivate)
--capi.client.connect_signal("request::border", permissions.update_border)
capi.client.connect_signal("request::activate", permissions.activate) -- <<--
--capi.client.connect_signal("request::tag", permissions.tag)
--capi.client.connect_signal("request::urgent", permissions.urgent)

--capi.client.connect_signal("request::geometry", permissions.geometry) -- ошибка при увеличении и уменьшение
--capi.client.connect_signal("request::geometry", permissions.merge_maximization)
--capi.client.connect_signal("request::geometry", permissions.client_geometry_requests)
--capi.client.connect_signal("property::border_width", repair_geometry)
--capi.client.connect_signal("property::screen", repair_geometry)
--capi.client.connect_signal("request::unmanage", check_focus_delayed)
...
capi.tag.connect_signal("property::selected",
                        function(t)
                            log("property::selected")
...
capi.client.connect_signal("mouse::press", function(c)
    --log("mouse::press")
end)
capi.client.connect_signal("mouse::release", function(c)
    --log("mouse::release")
end)
