...
-- @return None.
local function set_text(notification, title, text, appname)
    ...
    local function setMarkup(pattern, replacements)
        if not wmapi:is_empty(appname) then
            -- name app -> spotify -> Spotify
            local Appame = appname:sub(1, 1):upper() .. appname:sub(2)
            return textbox:set_markup_silently(string.format('<b>%s</b>\n<b>%s</b>%s', Appame, title, text:gsub(pattern, replacements)))
        else
    ...
    local function setText()
        if not wmapi:is_empty(appname) then
            -- name app -> spotify -> Spotify
            local Appame = appname:sub(1, 1):upper() .. appname:sub(2)
            textbox:set_text(string.format('%s\n%s %s', Appame, title, text))
        else
...
function naughty.notify(args)
    ...
    local freedesktop_hints = args.freedesktop_hints or args.freedesktop_hints
    local appname           = args.appname or ""

    --2022-05-11 00.40.50, [226383] DEBUG: i: destroy - v: function: 0x55579e878880
    --2022-05-11 00.40.50, [226383] DEBUG: i: replaces_id - v: 2037
    --2022-05-11 00.40.50, [226383] DEBUG: i: text - v: Трек **Let Me Down** було додано до черги.
    --2022-05-11 00.40.50, [226383] DEBUG: i: run - v: function: 0x55579ec55e60
    --2022-05-11 00.40.50, [226383] DEBUG: i: title - v: FredBoat♪♪ (#【☑】команды, • ☁ • Текстовые каналы)
    --2022-05-11 00.40.50, [226383] DEBUG: i: icon - v: lgi.rec 0x55579fd1a910:cairo.ImageSurface
    --2022-05-11 00.40.50, [226383] DEBUG: i: freedesktop_hints - v: table: 0x55579f5fbbd0
    --2022-05-11 00.40.50, [226383] DEBUG: i: preset - v: table: 0x55579ec55dc0
    --2022-05-11 00.40.50, [226383] DEBUG: i: actions - v: table: 0x55579ec55e10
    --2022-05-11 00.40.50, [226383] DEBUG: i: appname - v: discord
    ...
    set_text(notification, title, text, appname)
    ...
    if icon then
        ...
        if type(icon) == "string" and string.sub(icon, 1, 7) == "file://" then
            ...
            -- urldecode URI path
            icon = string.gsub(icon, "%%(%x%x)", function(x)
