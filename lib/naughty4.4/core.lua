...
local function init_screen(s)
	if naughty.notifications[s] then
		return
	end
...
function naughty.set_suspended(value)
	if properties["suspended"] == value then
		return
	end
...
function naughty.client_icon_handler(notification, context)
	if context ~= "clients" then
		return
	end

	local clients = notification:get_clients()

	for _, t in ipairs { "normal", "dialog" } do
		for _, c in ipairs(clients) do
			if c.type == t then
				notification._private.icon = gsurface(c.icon) --TODO support other size
				return
			end
		end
	end
...
function naughty.icon_path_handler(notification, context, hints)
	if context ~= "image" and context ~= "path" then
		return
	end

	notification._private.icon = gsurface.load_uncached_silently(
...
function naughty.icon_clear_handler(notification, context, hints)
